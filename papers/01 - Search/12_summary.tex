...
\begin{tabularx}{\textwidth}{lXX}
    ...
    \midrule
    \cite{trackgen} & \makecell{Research gate:\\Same author search\\(Pier Luca Lanzi)} & The previously mentioned surveys have only a small number of human subjects. The number of subjects may be increased by introducing an online line tool making their new approach available like they did in this paper.\\
    \midrule
    \cite{racingLine} & \makecell{IEEExplore search:\\Search string: ``racing game''} & It is not mentioned explicitly but this paper describes the same way encoding a track using Bezier curves and some of the authors are the same.\\
    \midrule
    \cite{dataDriven} & \makecell{IEEExplore search:\\Search string: ``racing game''} & This paper proposes an approach for generating new tracks based on given tracks with the same quality. This approach may be used to make the population of valid tracks grow faster.\\
    \midrule
    \cite{driverProfiling} & \makecell{IEEExplore search:\\Search string:\\``racing game''} & After generating a set of tracks the player may be classified to generate the next set of tracks based on his/her driving profile.\\
\end{tabularx}
}]
